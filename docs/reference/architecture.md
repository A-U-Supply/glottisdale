...
|------|-----------|--------|
| 1 | **Extract audio** -- symphonia decodes input to 16 kHz mono WAV | `audio::io` |
| 2 | **Transcribe** -- whisper-rs produces word-level timestamps (model auto-downloaded on first use) | `language::transcribe` |
...
| 8 | **Pitch normalization** -- estimate F0 via autocorrelation for each clip, compute the median, shift outliers toward it using Signalsmith Stretch | `audio::analysis` + `audio::effects` |
| 9 | **Volume normalization** -- compute RMS for each clip, shift to the median | `audio::analysis` + `audio::effects` |
...
| 13 | **Prosodic dynamics** -- apply onset boost (+1.12 dB over the first 20% of each phrase) and final softening (-3 dB from 70% onward) | `collage::process` |
...
| 16 | **Global speed adjustment** -- time-stretch the entire output using Signalsmith Stretch (pitch-preserving) | `audio::effects` |
...
The sing pipeline maps syllable clips onto MIDI melody notes to produce a "drunk choir" vocal track. It reuses the shared language modules for transcription and syllabification, then applies Signalsmith Stretch pitch shifting and time stretching to fit syllables to the melody.
...
| 3 | **Normalize syllable pitch** -- shift each syllable's F0 to the median using Signalsmith Stretch | `sing::syllable_prep` |
| 4 | **Normalize syllable volume** -- adjust each clip's RMS to the median | `sing::syllable_prep` (via `audio::effects`) |
...
| 6 | **Render each note** -- compute the semitone shift from median F0 to the MIDI note frequency, then apply Signalsmith Stretch pitch shift + time stretch to fit the note duration | `sing::vocal_mapper` |
| 7 | **Apply vibrato and chorus** -- vibrato on sustained notes; chorus via detuned/delayed voice layering | `sing::vocal_mapper` |
...
| `audio::effects` | Pitch shift and time stretch via Signalsmith Stretch, volume adjust, concatenation, crossfade, mixing, silence generation |
...
| `language::transcribe` | Native Whisper transcription via whisper-rs with automatic model download |
| `language::align` | Abstract `Aligner` trait, `DefaultAligner` (Whisper + g2p) |
...
| `sing::vocal_mapper` | Note-to-syllable mapping, per-note rendering with pitch/time stretch, vibrato, and chorus effects |
| `sing::synthesize` | MIDI sine-wave synthesizer with noise-burst drum synthesis |
| `sing::mixer` | Vocal + backing track mixing |
